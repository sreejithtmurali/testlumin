...
class _CartViewState extends State<CartView> {
  ...
  void openCheckout(String amt, String title, String discription) async {
    var options = {
      //dynamic key of clint please replace key with ur key
      'key': 'rzp_live_ILgsfZCZoFIKMb',
      //amt in pisa
      'amount': double.parse(amt) * 1,
      'name': title,
  ...
  Future<void> fetchCartItems() async {
    ...
    for (var item in items) {
      ...
      calculatedTotalPrice += (cartCount * (price*82));
     
    }
    String s=calculatedTotalPrice.toString();
    String priceWithoutDollar = s.replaceAll("\$", "");
    setState(() {
      cartItems = items;
      totalPrice = double.parse(priceWithoutDollar);
    });
  }
                ...
                Container(
                  ...
                  child: ElevatedButton(
                    onPressed: () {
                      ...
                      String priceWithoutDollar = s.replaceAll("\$,\u20B9", "");
                      openCheckout(priceWithoutDollar.trimLeft(), "cart items", "${cartItems.length} items checkout");
