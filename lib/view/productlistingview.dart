...
class _ProductsListingViewState extends State<ProductsListingView> {
  ...
  void _showScratcherAlert(BuildContext context, Products p) {
    showDialog(
        context: context,
        builder: (BuildContext context) {
          return AlertDialog(
            title: Text("Get This @ Rs:0!!!"),
            content: Container(
              height: 300,
              width: 300,
              child: Scratcher(
  ...
  Products getRandomItem(List<Products> list) {
    _userManager.incrementCount();
  ...
  Future<void> start(ProductLoaded state) async {
    int count = await _userManager.getCount();

    Timer(Duration(seconds: 3), () async {
      if (count == 0) {
        var p = await getRandomItem(state.plist);

        _showScratcherAlert(context, p);
      }
    });
  }
                              ...
                              builder: (BuildContext context) {
                                return Container(
                                  ...
                                  decoration: BoxDecoration(
                                    ...
                                    border: Border.all(
                                        width: .4, color: Color(0xffdddcdc)),
